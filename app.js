...
function runTest( id ){
    ...
    .then(function(){
        return validateConsistency( id, "Initial Save" )
    })
    .then(function(){
        console.log( "search bucket")
        return searchFind( bucket, "originalID:" + id )
    })
    .then(function(result){
        console.log( "remove from bucket")
        return  Promise.map( result[0].docs, function(x){
            return dbRemove(bucket, x.id );
        })
    })
    .then(function(){
